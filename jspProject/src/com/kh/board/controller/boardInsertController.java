...
public class boardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 일반방식이 아닌 multipart/form-data로 전송하는 경우는 request로 부터 값 뽑기 불가!!
		// String boardTitle = request.getParameter("title");
		
		// enctype이 mulitpart/form-data 로 잘 전송되었을 경우 전반적인 내용들이 수행되도록
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// 1_1) 전달되는 파일의 용량 제한 (int maxSize => byte 단위) => 10Mbyte 로 제한
			...
			/*
			 * >> HttpServletRequest request => MultipartRequest multiRequest 번환
			 * 
			 * 위 구문 한 줄 실행만으로 언오온 첨부파일이 해당 폴더에 무조건 업로드 됨
			 * 
			 * 단, 업로드시 파일명 수정해주는게 일반적임! 그래서 파일명 수정시켜주는 객체 제시해야됨
			 * => 같은 파일명이 존재할 경우 덮어씌워질수 있고, 파일명에 한글/특문/띄어쓰시가 포함 될 경우 서버에 따라 문제 발생
			 * 
			...
			// multipartRequest.getOriginalFileName("키"); : 넘어온 첨부파일이 있었을 경우 "원본
