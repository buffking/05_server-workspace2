...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 2) *session : 여기에 담긴 데이터는 내가 직접 지우기 전까지, 세션이 만료(서버 멈춤, 브라우저 종료)
		 ...
		 * 공통적으로 데이터를 꺼내고자 한다면 .getAttribute("key") : Object 타입으로 뱉음
		 * 공통적으로 데이터를 지우고자 한다면 .removeAttribute("key");
		 */
		...
		if(loginUser == null) {
			...
			// 조회결과가 있음 == 로그인 성공!! => 메인페이지 응답 (index.jsp)
			
			// 로그인한 회원정보(loginUser)를 session 에 담기! (여기저기서 가져다 쓸 수 있도록!)
			
			// Servlet에서는 session에 접근하고자 한다면 우선 session 객체 얻어와야함
			HttpSession session = request.getSession();
			
			session.setAttribute("loginUser", loginUser);
			
			// 1. 포워딩 방식 응답 뷰 출력
			//	  해당 선택된 jsp 보여질 뿐 url에는 여전히 현재 이 Servlet 매핑값이 남아있음
